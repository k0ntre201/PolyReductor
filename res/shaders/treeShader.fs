...
/*uniform sampler2D frontAlbedo;
uniform sampler2D backAlbedo;
...
float shadowCalculation(vec4 shadowTexCoord, vec3 N)
{
	...
	float bias = max(0.5 * (1.0 - dot(N,lightPos)), 0.005);
...
void main()
{
	...
	if(dot(N,lightPos)>0)
		color += (1.0 - s)*dot(N,lightPos)*c1;
	else
		color += abs(dot(N,lightPos))*c1*c2;
}
