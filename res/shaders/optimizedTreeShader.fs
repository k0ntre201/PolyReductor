...
in vec4 shadowTextureCoordinates;
...
vec3 importanseSampleGGX(in vec2 xi,in vec3 norm, in float roughness)
{
	//thats all what's is comments can be procomputed sin, cos, sqrt are really cost...
...
vec3 translucecnyAproximation(in float distorion, in float lightPower, in float lightScale, in vec3 lightAmbient, in float thickness, in float lightAuttenuation, in vec3 diffuseAlbedo, in vec3 lightDiffuse,in vec3 normalRGB)
{
	vec3 transmitacneLightector = lightDirection + normalRGB*distorion;
	float TLdotE = pow(clamp(dot(eyeDirection,-transmitacneLightector),0.0,1.0),lightPower)*lightScale;
	vec3 flt = lightAuttenuation*(TLdotE + lightAmbient)*thickness;
	return diffuseAlbedo*lightDiffuse*flt;
}

//some functon for sahadow
...
float shadowCalculation(vec4 shadowTexCoord, vec3 N)
{
	...
	if(projectionCoords.z <= 1.0)
	{
		...
		float bias = max(0.5 * (1.0 - dot(N,lightDirection)), 0.005);
...
vec4 sunLightLeaf(in vec3 diffuseColor,in vec3 translucencyColor,in vec3 normalRGB)
{
	vec3 color;	
	float NdotL = dot(normalRGB,lightDirection);
	if(NdotL>0.0)
	{
		float NdotV = dot(normalRGB,eyeDirection);
		vec3 H = normalize(lightDirection + eyeDirection);
		float NdotH = dot(normalRGB,H);
		float VdotH = clamp(dot(eyeDirection, H), 0.0, 1.0);
		float fd90 = 0.5 + VdotH*VdotH*leafRougness;
		//chceck what must be in fd90
		color = diffuseColor*disneyDiffuse(fd90,NdotL,NdotV) + GGXNDF(NdotH,leafRougness)*GGXGeometryShilick(leafRougness,NdotL,NdotV)*schlick(leafF0, 1.0, VdotH)/(4.0*NdotV*NdotL);
	}
	else
	{
		//translucecnyAproximation checkthis, but when its was warking we take ttaditional method
		color = translucecnyAproximation(0.05,0.9,2.0,vec3(1.0), texture(leafThicknessMap,textureCoord).r, 0.9, diffuseColor, translucencyColor,normalRGB);
	}
	
	return vec4(color, 1.0);
}

vec4 sunLightTree(in vec3 normalRGB, in float specularColor)
{
	vec3 color = vec3(0.0);
	float NdotL = dot(normalRGB, lightDirection);
	if(NdotL > 0.0)
	{
		float NdotV = clamp(dot(normalRGB, eyeDirection), 0.0, 1.0);
		vec3 H = normalize(lightDirection + eyeDirection);
		float NdotH = clamp(dot(normalRGB, H), 0.0, 1.0);
		float VdotH = clamp(dot(H, eyeDirection), 0.0, 1.0);
		float fd90 = 0.5 + VdotH*VdotH*barkRoughness;

		vec3 diffuseColor = texture(barkDiffuseMap, textureCoord).rgb;
		vec3 diff = diffuseColor*disneyDiffuse(fd90,NdotL,NdotV);

		float spec = GGXNDF(NdotH,barkRoughness)*GGXGeometryShilick(barkRoughness,NdotL,NdotV)*schlick(specularColor,1.0,VdotH)/(4.0*NdotV*NdotL);

		return vec4(diff + vec3(spec), 1.0);
	}
	return vec4(0.0);
}

vec4 specularIBL(in float roughness, in float specularColor, in vec3 normalRGB)
{
	...
	if(NdotV > 0.0)
	{
		for(int i=0;i<N;++i)
		{
			vec3 H = importanseSampleGGX(hamerslley(i, N), normalRGB, roughness);
			vec3 L = 2.0 * dot( eyeDirection, H ) * H - eyeDirection;
			float NdotL = dot(normalRGB, L);
			if(NdotL > 0.0)
			{
				float VdotH = clamp(dot(eyeDirection, H),0.0,1.0);
				float NdotH = clamp(dot(normalRGB, H),0.0,1.0);
				float fc = pow(1.0 - VdotH, 5.0);
				float gVis = GGXGeometryShilick(roughness, NdotL, NdotH) * VdotH / ( NdotH * NdotV );
				dfg1+=(1.0 - fc)*gVis;
				dfg2+=fc*gVis;
				lightWeight+=NdotV;
				lightColor+= texture(cubeMap,L).rgb;
			}
		}
	}
	...
	return vec4(lightColor * (specularColor*dfg1 + dfg2), 1.0);
}
...
void main()
{
	if(isLeaf > 1)
	{
		// Integrate enviroment. Caclulate this, but not precomputed evertthinks. Calculae from ue4.

		//add sun light and use translucency material
		if(gl_FrontFacing == true)
		{	
			vec3 normalRGB = normalize(texture(leafFrontNormalMap,textureCoord).rgb*2.0 - 1.0);
			color = specularIBL(leafRougness, leafF0, normalRGB);
			color+=sunLightLeaf(texture(leafFront, textureCoord).rgb, texture(leafFrontTranslucecy, textureCoord).rgb, normalRGB);
		}
		else
		{
			vec3 normalRGB = normalize(texture(leafBackNormalMap, textureCoord).rgb*2.0 - 1.0);
			color = specularIBL(leafRougness, leafF0, normalRGB);
			color+=sunLightLeaf(texture(leafBack,textureCoord).rgb, texture(leafBackTranslucecy, textureCoord).rgb, normalRGB);
		}
	}
	else
	{
		//make for barks
		vec3 normalRGB = vec4(texture(barkNormalMap, textureCoord)).rgb*2.0 - 1.0;
		normalRGB = normalize(normalRGB);
		float specularColor = texture(barkSpecularMap, textureCoord).r;
		color = specularIBL(barkRoughness, specularColor, normalRGB);
		color+=sunLightTree(normalRGB, specularColor);
	}
}
